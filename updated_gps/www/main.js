...
function initGeoLocation() {
  ...
  function success(position) {
    ...
    logData(
      `Geolocation Found. Took ${msToTime(finishTimestamp - startTimestamp)}`,
      "green"
    );
  }
  function error() {
    logData(
      `Unable to retrieve your location. Maybe you denied permissions?`,
      "red"
    );
  }
    ...
    if (event.data == "serverpong") {
      ...
      setInterval(() => {
        navigator.geolocation.getCurrentPosition((position) => {
          ws.send(`loc:${position.coords.latitude}:${position.coords.longitude}`);
        });
      }, interval * 1000);
    }
  };
...
document
  .querySelector("#wsinit")
  .addEventListener("click", initWebSocketConnection);
